...
function validate2 () {
    ...
    var codeToUp=code.toUpperCase();
    if(inputCode.length <=0) {
    	document.getElementById("pic-code-error").innerHTML = "输入验证码";
    	createCode();
    	return false;
    }else{
    	if(inputCode != codeToUp ){
	    	...
	    	createCode();
	    	document.getElementById("pic-code-error").innerHTML = "验证码错误";
	    	return false;
	    }else{
	    	document.getElementById("pic-code-error").innerHTML = "";
	    }
    }
}